...
dryrun_server() {
  ...
  # Bring down VM NIC interface
  ip link set $NIC_IFACE down
...
dryrun_tgen() {
    ...
    cat <<EOF
  # Start vaigai on mlx5
  "$VAIGAI_BIN" -l 14-15 -n 4 -a 0000:95:00.0 -a $QAT_PF_VAIGAI -- -I 10.0.0.1
EOF
}
...
dryrun_cleanup() {
  ...
  # Rebind VM NIC back to mlx5_core
  echo $NIC_VM > /sys/bus/pci/devices/$NIC_VM/driver/unbind
  echo "" > /sys/bus/pci/devices/$NIC_VM/driver_override
  echo $NIC_VM > /sys/bus/pci/drivers/mlx5_core/bind
